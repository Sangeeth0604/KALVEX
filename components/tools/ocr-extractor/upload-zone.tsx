...
import {
  MAX_OCR_FILE_SIZE_LABEL,
  validateOcrFile,
} from "@/lib/tools/ocr-extractor/ocr-engine";
import { OcrError } from "@/lib/tools/ocr-extractor/types";
...
export function UploadZone({
  ...
  return (
    <div
      ...
      aria-label="Upload image or PDF document for OCR text extraction"
    >
      <input
        ref={fileInputRef}
        type="file"
      ...
      <div className="h-14 w-14 rounded-xl bg-surface-raised border border-border-default flex items-center justify-center text-accent mb-4 transition-transform group-hover:scale-105 shadow-subtle">
        ...
        >
          ...
          <path d="M9 20h6" />
          <path d="M12 4v16" />
          <rect x="2" y="2" width="20" height="20" rx="2" strokeDasharray="3 3" />
        </svg>
      </div>

      <h3 className="text-base sm:text-xl font-bold text-text-primary mb-1.5">
        Upload image or PDF to extract text
      </h3>

      <p className="text-xs sm:text-sm text-text-secondary max-w-md mb-6 leading-relaxed">
        Select a document or image to perform Optical Character Recognition (OCR) locally in your browser. No files are uploaded to any external server.
      </p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 text-[11px] font-mono text-text-muted">
        ...
        <span>•</span>
        <span className="bg-surface-raised px-2.5 py-0.5 rounded border border-border-subtle">
          Max {MAX_OCR_FILE_SIZE_LABEL}
        </span>
        <span>•</span>
        <span className="text-accent font-medium">100% In-Browser WASM</span>
      </div>
    </div>
  );
}
